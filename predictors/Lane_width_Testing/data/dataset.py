...
import numpy as np
import cv2
from predictors.Lane_width_Testing.data.mytransforms import find_start_pos
...
class LaneClsDataset(torch.utils.data.Dataset):
    def __init__(self, path, list_path, img_transform=None, target_transform=None, simu_transform=None, griding_num=50, 
                 load_name=False, row_anchor=None, use_aux=False, segment_transform=None, num_lanes=4):
    ...
    def __getitem__(self, index):
        ...
        w, h = img.size
        cls_label = self._grid_pts(lane_pts, self.griding_num, w)  # make the coordinates to classification label
        if self.use_aux:
    ...
    def _get_index(self, label):
        ...
        for i in range(self.num_lanes):
            if np.all(all_idx_cp[i, :, 1] == -1):
                continue  # if there is no lane

            valid = all_idx_cp[i, :, 1] != -1  # get all valid lane points' index
            valid_idx = all_idx_cp[i, valid, :]  # get all valid lane points
            if valid_idx[-1, 0] == all_idx_cp[0, -1, 0]:
                continue  # skip if the last valid lane point is already at the bottom boundary
            if len(valid_idx) < 6:
                continue  # if the lane is too short to extend
